...
sed -i "/^lighttpd /i start-stop-daemon --start --background --name cloudflared --chdir \/etc\/pihole --exec \/usr\/local\/bin\/cloudflared -- --config \/etc\/pihole\/cloudflared.yml" /etc/s6-overlay/s6-rc.d/lighttpd/run
